#!/bin/bash
ptyxis -- /bin/sh -c 'python /var/home/$USER/Documents/code/latin-leven/wiktionary_latin_py/wiktionary_latin.py |
  tee /dev/tty | {
...
# /bin/sh -c is there so that all the single-quoted commands run together in the new ptyxis window
